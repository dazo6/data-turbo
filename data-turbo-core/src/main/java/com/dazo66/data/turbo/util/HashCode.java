...
public abstract class HashCode {
    ...
    /**
     * Returns the first four bytes of {@linkplain #asBytes() this hashcode's bytes}, converted
     * to an
    ...
    /**
     * Returns the first eight bytes of {@linkplain #asBytes() this hashcode's bytes}, converted
     * to a
     ...
     * example, you can safely put {@code HashCode} instances into a {@code HashSet}) but is
     * otherwise
     ...
     *
     * <p><b>Security note:</b> this method uses a constant-time (not short-circuiting)
     * implementation
     ...
     *
     * <p>Note that if the output is considered to be a single hexadecimal number, whether this
     * string
    ...
    private static final class LongHashCode extends HashCode implements Serializable {
        ...
        public byte[] asBytes() {
            return new byte[]{(byte) hash, (byte) (hash >> 8), (byte) (hash >> 16),
                    (byte) (hash >> 24), (byte) (hash >> 32), (byte) (hash >> 40),
    ...
    private static final class BytesHashCode extends HashCode implements Serializable {
        ...
        @Override
        public int asInt() {
            checkState(bytes.length >= 4, "HashCode#asInt() requires >= 4 bytes (it only has %s " +
                    "bytes).", bytes.length);
        ...
        @Override
        public long asLong() {
            checkState(bytes.length >= 8, "HashCode#asLong() requires >= 8 bytes (it only has %s " +
                    "bytes).", bytes.length);
            return padToLong();
        }
