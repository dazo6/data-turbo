...
public class Preconditions {
     ...
     *                             message is formed by replacing each {@code %s} placeholder in
     ...
     *                             appended to the formatted message in
    ...
    public static void checkArgument(boolean expression, String errorMessageTemplate,
        ...
        if (!expression) {
            throw new IllegalArgumentException(Strings.lenientFormat(errorMessageTemplate,
                    errorMessageArgs));
        }
    }
    ...
    public static void checkState(boolean expression,
             ...
             * template to be null (though we do handle that case gracefully at runtime). I've
             * left this
             * one as it is because one of our users has defined a wrapper API around Preconditions,
             * declaring a checkState method that accepts a possibly null template. So we'd need
             * to update
             * that user first.
             */
                                  String errorMessageTemplate, Object... errorMessageArgs) {
        if (!expression) {
            throw new IllegalStateException(Strings.lenientFormat(errorMessageTemplate,
                    errorMessageArgs));
        }
    }
    ...
    /*
     * Preconditions.checkNotNull is *intended* for performing eager null checks on parameters
     * that a
     ...
     * decade that assumes that they can use checkNotNull for non-precondition checks. I had
     * hoped to
     ...
     *
     * We still encourage people to use requireNonNull over checkNotNull for non-precondition
     * checks.
     */
    ...
    public static <T> T checkNotNull(T reference, String errorMessageTemplate,
                                     Object... errorMessageArgs) {
        if (reference == null) {
            throw new NullPointerException(Strings.lenientFormat(errorMessageTemplate,
                    errorMessageArgs));
        }
        return reference;
    }
     ...
     * slower.
     * This is a big deal - we're talking factors of 2-8 in microbenchmarks, not just 10-20%.
     * (This is
     ...
     *
     * But the methods in this class want to throw different exceptions, depending on the args,
     * so it
     * appears that this pattern is not directly applicable. But we can use the ridiculous, devious
     * trick of throwing an exception in the middle of the construction of another exception.
     * Hotspot
     * is fine with that.
     */
    ...
    private static String badPositionIndex(int index, int size, String desc) {
        if (index < 0) {
            ...
            return Strings.lenientFormat("%s (%s) must not be greater than size (%s)", desc,
                    index, size);
        }
    }

    /**
     * Ensures that {@code start} and {@code end} specify valid <i>positions</i> in an array,
     * list or
    ...
    private static String badPositionIndexes(int start, int end, int size) {
        ...
        // end < start
        return Strings.lenientFormat("end index (%s) must not be less than start index (%s)", end
                , start);
    }
}
