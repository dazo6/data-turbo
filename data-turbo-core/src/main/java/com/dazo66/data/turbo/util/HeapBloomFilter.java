 ...
 */
public class HeapBloomFilter<T extends Object> implements Predicate<T>, IBloomFilter<T>,
        Serializable {
    /**
     ...
     */
    private HeapBloomFilter(BloomFilterStrategies.LockFreeBitArray bits, int numHashFunctions,
     ...
     * @since 12.0
     */
    public HeapBloomFilter<T> copy() {
        return new HeapBloomFilter<T>(bits.copy(), numHashFunctions, funnel, strategy);
    }
    ...
    public void writeTo(OutputStream out) throws IOException {
        ...
        dout.writeByte(ByteUtils.unsignedCheckedCast(numHashFunctions));
        dout.writeLong(bits.data.length() * 64L);
    ...
    public long approximateElementCount() {
         ...
         * `numHashFunctions/bitSize`. So, after n insertions, expected bitCount is `bitSize * (1
         * - (1 -
         * numHashFunctions/bitSize)^n)`. Solving that for n, and approximating `ln x` as `x - 1`
         * when x
     ...
     * @since 15.0
     */
    public boolean isCompatible(HeapBloomFilter<T> that) {
        checkNotNull(that);
     ...
     * @since 15.0
     */
    public void putAll(HeapBloomFilter<T> that) {
        checkNotNull(that);
        ...
        checkArgument(this.strategy.equals(that.strategy), "BloomFilters must have equal " +
                "strategies (%s != %s)", this.strategy, that.strategy);
        checkArgument(this.funnel.equals(that.funnel), "BloomFilters must have equal funnels (%s " +
                "!= %s)", this.funnel, that.funnel);
    ...
    public boolean equals(Object object) {
        ...
        if (object instanceof HeapBloomFilter) {
            HeapBloomFilter<?> that = (HeapBloomFilter<?>) object;
