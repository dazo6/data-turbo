...
public abstract class Converter<A, B> implements Function<A, B> {
     ...
     * cannot be
     * converted, an unchecked exception (such as {@link IllegalArgumentException}) should be
     * thrown.
     *
    ...
    /**
     * Returns a representation of {@code b} as an instance of type {@code A}. If {@code b}
     * cannot be
     * converted, an unchecked exception (such as {@link IllegalArgumentException}) should be
     * thrown.
     *
     ...
     *                                       then this is not logically a {@code Converter} at
     *                                       all, and should just implement {@link
     *                                       Function}.
     */
     ...
     * documented as being for use with type parameters that have parametric nullness. But
     * Converter's
     * type parameters do not. Still, we use it here so that we can suppress a warning at a smaller
     * level than the whole method but without performing a runtime null check. That way, we can
     * still
     ...
     *
     * But no matter what we do, it's worth remembering that the resulting code is going to be
     * unsound
     * in the presence of LegacyConverter, at least in the case of users who view the converter as a
     * Function<A, B> or who call convertAll (and for any checkers that apply @PolyNull-like
     * semantics
     ...
     *
     * For convertAll, I haven't looked as closely at *how* much existing users benefit, so we
     * should
     * keep an eye out for problems that new users encounter. Note also that convertAll could
     * support
     * both use cases by using @PolyNull. (By contrast, we can't use @PolyNull for our
     * superinterface
        ...
        return new Iterable<B>() {
            ...
            public Iterator<B> iterator() {
                return new Iterator<B>() {
                    ...
                    // See code comments on convertAll and Converter.apply.
     ...
     * discussed in a code comment at the top of the class), we declare our override of Function
     * .apply
     ...
     *   have null returned to them. But our signature isn't making their existing nullness type
     * error
     *   any worse.)
     ...
     *   bytecode. Notably, NullPointerTester expects a method to throw NPE when passed null
     * unless it
     *   is annotated in a way that identifies its parameter type as potentially including null.
     * (And
     *   this method does not throw NPE -- nor do we want to enact a dangerous change to make it
     * begin
     ...
     *   parameter and return type here, then anyone who used such a tool (and managed to pass
     * null to
     *   this method, presumably because that user doesn't run a normal nullness checker) could see
     *   NullPointerException.
     *
     * [*] Granted, such tools could conceivably be smart enough to recognize that the apply()
     * method
     * on a a Function<Foo, Bar> should never allow null inputs and never produce null outputs
     * even if
     * this specific subclass claims otherwise. Such tools might still produce NPE for calls to this
     * method. And that is one reason that we should be nervous about "lying" by extending
     * Function<A,
     * B> in the first place. But for now, we're giving it a try, since extending Function<@Nullable
     * A, @Nullable B> will cause issues *today*, whereas extending Function<A, B> causes
     * problems in
     * various hypothetical futures. (Plus, a tool that were that smart would likely already
     * introduce
     ...
     * {@link Object#equals}.
     * However, an implementation may also choose to return {@code true} whenever {@code object}
     * is a
     ...
     * true for
     * all {@code a} of type {@code A} (and similarly for {@code reverse}). Note that a {@code
     * false}
